...
export function DeckPerformanceTable({ data }: { data: DeckPerformance[] }) {
  if (data.length === 0) {
    return <p className="text-sm text-ink-500 text-center py-8">No deck data yet</p>
  }
      ...
      {data.map((d) => (
        <div key={d.deck_id} className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="font-medium text-ink-100">{d.deck_name}</span>
            <span className="text-ink-500 font-mono text-xs">{d.card_count} cards</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-ink-400 w-20 text-right">
