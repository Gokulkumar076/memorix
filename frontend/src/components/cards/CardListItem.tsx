...
const stateVariant: Record<string, 'synapse' | 'recall' | 'decay' | 'mint' | 'neutral'> = {
  new: 'recall',
  learning: 'decay',
  review: 'mint',
  relearning: 'decay',
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <p className="text-sm text-void-100 line-clamp-2 mb-2">{previewText}</p>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {card.tags.map((tag) => (
            <span key={tag} className="text-xs text-void-500 font-mono">#{tag}</span>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        ...
        <button onClick={() => onEdit(card)} className="p-2 rounded-lg text-void-400 hover:text-ghost hover:bg-white/5">
          <Edit3 className="h-3.5 w-3.5" />
        </button>
        <button onClick={() => onDelete(card)} className="p-2 rounded-lg text-void-400 hover:text-decay-300 hover:bg-decay-500/10">
